...
module.exports = class MailService extends Service {
    ...
    async sendCsvExport(email, csvContent) {
        const info = await this.transporter.sendMail({
            from: '"IUT App" <no-reply@iut-app.com>',
            to: email,
            subject: 'Movies Export CSV',
            text: 'Here is the CSV export of all movies you requested.',
            html: '<b>CSV Export Available!</b><br/>Please find the attached CSV file with all movies.',
            attachments: [
                {
        ...
        console.log('CSV Export Email sent: %s', info.messageId);
        console.log('Preview URL: %s', nodemailer.getTestMessageUrl(info));
